...
export interface ILinksClient {
    ...
    editLink(id: number, command: EditLinkCommand): Observable<void>;
...
export class LinksClient implements ILinksClient {
    ...
    editLink(id: number, command: EditLinkCommand): Observable<void> {
        ...
        return this.http.request("put", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processEditLink(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processEditLink(response_ as any);
                } catch (e) {
    ...
    protected processEditLink(response: HttpResponseBase): Observable<void> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
...
export class EditLinkCommand implements IEditLinkCommand {
    id?: number;
    shortUrlCode?: string;

    constructor(data?: IEditLinkCommand) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): EditLinkCommand {
        data = typeof data === 'object' ? data : {};
        let result = new EditLinkCommand();
        result.init(data);
        return result;
    }
...
export interface IEditLinkCommand {
    id?: number;
    shortUrlCode?: string;
}
